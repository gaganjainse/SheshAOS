//! `sheshaaos init` — Initialize a new SheshaAOS data directory.
...
pub fn run(config_path: &str) -> Result<(), NexusError> {
    info!("Initializing SheshaAOS with config: {}", config_path);
...
"#;
    ...
    println!("✓ SheshaAOS initialized at {}", data_dir.display());
    Ok(())
}
