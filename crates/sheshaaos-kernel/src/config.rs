//! Application configuration for SheshaAOS.
//!
...
pub struct GeneralConfig {
    /// Directory where SheshaAOS stores events, snapshots, and artifacts.
    pub data_dir: String,
...
"#
    ...
    fn test_load_from_string() {
        ...
        assert_eq!(config.general.data_dir, "/tmp/sheshaaos-test");
...
"#;
    ...
    fn test_resolved_data_dir_absolute() {
        let config = AppConfig::parse_toml(sample_toml()).expect("should parse");
        assert_eq!(config.resolved_data_dir(), PathBuf::from("/tmp/sheshaaos-test"));
    }

    #[test]
    fn test_resolved_data_dir_tilde() {
        let toml = sample_toml().replace("/tmp/sheshaaos-test", "~/.sheshaaos");
