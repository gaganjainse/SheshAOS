...
impl TerminalState {
    ...
    pub fn wire_pty(&mut self) {
        ...
        let pair = match pty_system.openpty(PtySize {
            ...
            Ok(p) => p,
            Err(e) => {
                eprintln!("[SheshaAOS] PTY open failed: {e}");
                return;
            }
        };
        ...
        if let Err(e) = pair.slave.spawn_command(cmd) {
            eprintln!("[SheshaAOS] Shell spawn failed: {e}");
            return;
        }
        ...
        match pair.master.take_writer() {
            ...
            Err(e) => eprintln!("[SheshaAOS] PTY writer failed: {e}"),
        }
        ...
        let mut reader = match pair.master.try_clone_reader() {
            Ok(r) => r,
            Err(e) => {
                eprintln!("[SheshaAOS] PTY reader failed: {e}");
                return;
            }
        };
