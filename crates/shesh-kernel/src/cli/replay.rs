//! `shesh replay` — Replay event history for a task.

use std::str::FromStr;

use tracing::info;

use crate::{config::AppConfig, error::KernelError, storage::SqliteEventStore, task::TaskId};

/// Replay and display the event history for a given task ID.
pub fn run(config_path: &str, task_id: &str) -> Result<(), KernelError> {
    ...
    rt.block_on(async {
        ...
        let store = SqliteEventStore::open(events_dir).await?;

        let events = if task_id == "all" {
            store.read_all().await?
        } else {
            let id = TaskId::from_str(task_id).map_err(|_| {
                KernelError::Task(crate::error::TaskError::NotFound { id: task_id.to_string() })
            })?;
