//! `nexusaos init` — Initialize a new NexusAOS data directory.
...
pub fn run(config_path: &str) -> Result<(), NexusError> {
    info!("Initializing NexusAOS with config: {}", config_path);
...
"#;
    ...
    println!("✓ NexusAOS initialized at {}", data_dir.display());
    Ok(())
}
